...
def makeLib(file_path, protein_name, residue_name, connect0=None, connect1=None, charges='bcc', atom_type='gaff', force_field='leaprc.ff12SB', parameterized=False):
	...
	if not parameterized:
		#Execute  #create parametrizes file stuff: PDB_tmp, mol2 and so on.
		subprocess.call(["antechamber -i %s -fi %s -nc 2.02 -o %s.mol2 -fo mol2 -c %s -rn %s -at %s"%(file_path,
																						extension, 
																						name, 
																						charges, 
																						residue_name, 
	...
	#load PDB info into PDBfile object;
